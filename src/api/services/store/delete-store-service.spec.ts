...
describe("delete a store", () => {
  ...
  it("should not be able to delete a store if the id store token is different from the id", async () => {
    ...
    expect(store.value.message).toBe("Você não tem permissão para deletar essa loja");
  });

  it("should not be able to delete a store if the id field is empty", async () => {
    await createStoreService.execute("mtCompany", "mtcompany@teste.com", "102030");

    const store = await deleteStoreService.execute();
  ...
  it("should not be able to delete a store if the id field is different type of number", async () => {
    await createStoreService.execute("mtCompany", "mtcompany@teste.com", "102030");
  ...
  it("should not be able to delete a store if the id field is less then 1", async () => {
    await createStoreService.execute("mtCompany", "mtcompany@teste.com", "102030");
